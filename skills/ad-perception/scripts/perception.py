...
def _service_log_scan_segments(
    ...
    level_values: List[str] = [str(level) for level in (levels or []) if str(level).strip()]
    module_values: List[Optional[str]] = modules or [None]
    segments = []
    for module in module_values:
        segment = {"skip": 0, "done": False, "scanned": 0}
        if level_values:
            segment["levels"] = level_values
        if module:
...
def _service_log_page_kwargs(job: Dict[str, Any], segment: Dict[str, Any]) -> Dict[str, Any]:
    kwargs: Dict[str, Any] = {"limit": int(job.get("page_size") or 1000), "skip": int(segment.get("skip") or 0)}
    if job.get("from_time"):
        kwargs["from_time"] = job["from_time"]
    if job.get("to_time"):
        kwargs["to_time"] = job["to_time"]
    if segment.get("levels"):
        kwargs["levels"] = [str(level) for level in segment["levels"] if str(level).strip()]
    elif segment.get("level"):
...
def main() -> None:
    ...
    logs_start_p.add_argument("--page-size", type=int, default=1000, help="Rows per device API page")
