...
The version comes from `package.json`. The current handoff baseline is `0.5.5`; after this release, increment patch versions by `0.0.1` for each follow-up delivery unless the user explicitly asks for a different version.
