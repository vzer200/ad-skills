...
- Scoring rule: pass = 1, warn = 0.5, fail = 0; `not_applicable` is displayed as `➖ 不适用` and excluded from score denominators, anomaly counts, and optimization suggestions. `分类统计` uses `检查项 / ✅ 正常 / ❌ 异常 / ➖ 不适用 / 得分`; `检查项` includes not-applicable rows so category totals reconcile with the progress total, while `得分` still excludes not-applicable rows from its denominator. Empty dimensions must not pull down the overall score; the overall score averages only dimensions that appear in the current report.
