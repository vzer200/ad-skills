...
- R3 visible output must preserve the `perception.py` markdown block instead of rewriting conclusions; do not add phrases such as `大幅偏离`, `连接数约为基线`, or `降至 0` unless they are emitted by the script itself.
