...
The renderer covers the deterministic command form for AD-OPS generated plans.
When provided a read-only preflight result, it may skip create commands for
objects that already exist and render rollback commands from preflight
snapshots. It does not perform CLI login or execute commands.
