...
| # | 严禁 | 正确做法 |
|---|------|----------|
| 1 | **二次渲染脚本输出** — 对 `wait` / `analyze` 输出的 Markdown 做重新格式化、截取、改写、表格重排、补充摘要 | **原样展示**直接把脚本输出内容展示在对话消息正文中 |
...
| 7 | **A2 擅自决定巡检场景** — 用户未指定场景时，LLM 自行默认"标准巡检"而不询问 | **【询问用户】**确认场景后再继续 |
| 8 | **A5 跳过 progress 轮询直接调 `wait`** — 以为 `wait` 会自己等，实际有 60s 硬超时 | 必须每 10s 手动轮询 `progress`，确认 `FINISHED` 后再调 `wait` |
...
> ⛔ **禁止跳过此步骤直接调 `wait`**。`wait` 有 60s 硬超时，巡检通常需要更长时间，直接调 `wait` 必然超时。
