...
- 场景未确定前只允许读取技能说明、设备清单或调用 `check.py prompt --stage scene` 做轻量确认；禁止提前连接设备、执行 `overview.py` 或产出查询结果。用户补充场景后，必须先执行 `connect.py -> check.py history`，然后询问是否强制继续；用户确认强制继续后，才执行 `run -> progress -> wait`。
...
- 交互硬停规则：用户只回复 `标准巡检`、`全量巡检` 或 `安全巡检` 时，必须先执行 `connect.py` 和 `check.py history` 查询真实设备历史，然后最终正文只能询问是否强制继续；禁止执行 `check.py run`、`check.py progress`、`check.py wait`，禁止输出报告。只有用户随后明确回复 `强制`、`继续` 或 `强制继续` 后，才允许启动巡检。
