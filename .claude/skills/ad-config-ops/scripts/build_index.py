...
SUMMARY_KEYS = ("enum", "optionalEnum", "default", "example", "readOnly")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--skill-root", required=True, type=Path, help="ad-config-ops skill root containing references/api-docs.")
    ...
    if isinstance(schema.get("items"), dict):
        ...
        for key in ("type", "format", "description"):
            if key in items and items[key] is not None:
                item_summary[key] = items[key]
        if isinstance(items.get("$ref"), str):
            item_summary["ref"] = ref_tail(items["$ref"])
