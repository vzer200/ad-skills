...
function packPublicBase(options = {}) {
  ...
  const dirtyPublicInputs = collectDirtyPublicInputs(repoRoot, config);
  if (dirtyPublicInputs.length > 0) {
    warnings.push({
      type: 'dirty_public_inputs_after_full_build',
      count: dirtyPublicInputs.length,
      sample: dirtyPublicInputs.slice(0, 200)
    });
  }
  ...
  const manifest = {
    ...
    dirty_public_inputs_count: dirtyPublicInputs.length,
    dirty_public_inputs_after_full_build: dirtyPublicInputs.slice(0, 200),
...
function publishPublicBaseWithGit(options = {}) {
  ...
  gitOk(cache.path, ['add', '--', branchPath], { exitCode: 4 });
  ensureLocalGitIdentity(cache.path, { exitCode: 4 });
  const status = gitOk(cache.path, ['status', '--porcelain'], { exitCode: 4 }).stdout.trim();
  if (!status) {
    const result = {
      ...published,
      status: 'no_changes',
  ...
  gitOk(cache.path, ['commit', '-m', `publish public-base for ${branchPath}`], { exitCode: 4 });
  gitOk(cache.path, ['push', '-u', 'origin', 'HEAD'], { exitCode: 4 });
  const commit = gitOk(cache.path, ['rev-parse', 'HEAD'], { exitCode: 4 }).stdout.trim();
  const result = {
    ...published,
    status: 'published',
    repo: cache.url,
...
function ensureManagedArtifactRepo(repoRoot, options = {}) {
  ...
  if (!fs.existsSync(path.join(cachePath, '.git'))) {
    ...
    gitOk(repoRoot, ['clone', url, cachePath], { exitCode: 4 });
    ensureManagedCachePathSafe(repoRoot, cachePath);
  } else {
    const remote = gitOk(cachePath, ['remote', 'get-url', 'origin'], { exitCode: 4 }).stdout.trim();
    ...
    const branch = gitOk(cachePath, ['branch', '--show-current'], { exitCode: 4 }).stdout.trim();
    const fetch = git(cachePath, ['fetch', 'origin']);
    if (fetch.status !== 0) {
      const error = new Error(`git fetch failed: ${sanitize(fetch.stderr || fetch.stdout || fetch.error?.message || fetch.status)}`);
      error.exitCode = 4;
      throw error;
    }
    if (branch) {
      const reset = git(cachePath, ['reset', '--hard', `origin/${branch}`]);
      if (reset.status === 0) {
        gitOk(cachePath, ['clean', '-fdx'], { exitCode: 4 });
      } else {
        const error = new Error(`git reset failed: ${sanitize(reset.stderr || reset.stdout || reset.error?.message || reset.status)}`);
        error.exitCode = 4;
        throw error;
      }
    }
...
function runCheck(options = {}) {
  ...
  try {
    ...
    if (options.integrityOnly) {
      ...
      const cleanPublicInputs = dirtyPublicInputs.length === 0;
      const dirtyInputsBlockReuse = config.public_input_mode !== 'worktree' && !cleanPublicInputs;
      result = {
        schema_version: 1,
        generated_at: core.nowIso(),
        ...
        dirty_public_inputs_count: dirtyPublicInputs.length,
        dirty_public_inputs_sample: dirtyPublicInputs.slice(0, 200),
        ...
        warnings: [
          ...
          ...(cleanPublicInputs ? [] : [{
            message: config.public_input_mode === 'worktree'
              ? 'dirty public inputs are included because public_input_mode is worktree; use git-head for trusted publish baselines'
              : 'dirty public inputs changed; rebuild public-base or commit/revert public inputs before trusting local app verification'
          }]),
          ...sidecar.warnings
        ]
      };
...
function runPublicBaseCli(args = [], options = {}) {
  ...
  try {
    ...
    if (command === 'auth') {
      ...
      const result = runAuthCommand(subcommand, { ...parsed, repoRoot: options.repoRoot || parsed.repoRoot, cwd: options.cwd, env: options.env, stdin: options.stdin || process.stdin });
      writeCliResult(stdout, parsed, result, `public-base auth ${subcommand} ${result.status}\n`);
...
function gitOk(repoRoot, args, options = {}) {
  ...
  if (result.error || result.status !== 0) {
    const error = new Error(`git ${args[0]} failed: ${sanitize(result.error?.message || result.stderr || result.stdout || result.status)}`);
    if (options.exitCode) {
...
function runAuthCommand(command, options = {}) {
  const repoRoot = resolveRepoRoot(options);
  const env = options.env || process.env;
  const repo = publicBaseRepoUrl({ env });
  if (command === 'login') {
    if (!options.tokenStdin) {
      const error = new Error('auth login requires --token-stdin');
      error.exitCode = 2;
      throw error;
    }
    const token = fs.readFileSync(0, 'utf8').replace(/\r?\n$/, '');
    if (!token) {
      const error = new Error('empty token');
      error.exitCode = 2;
      throw error;
    }
    approveCredential(repo, token);
    return {
      status: 'stored',
      repo,
  ...
  if (command === 'status') {
    const result = spawnSync('git', ['ls-remote', repo, 'HEAD'], { cwd: repoRoot, encoding: 'utf8', maxBuffer: GIT_MAX_BUFFER });
    return {
      status: result.status === 0 ? 'authenticated' : 'unauthenticated',
      repo,
      error: result.status === 0 ? null : sanitize(result.stderr || result.stdout || result.error?.message || 'git ls-remote failed')
    };
  }
  if (command === 'logout') {
    rejectCredential(repo);
...
function approveCredential(repo, token) {
  ...
  const result = spawnSync('git', ['credential', 'approve'], { input, encoding: 'utf8', maxBuffer: GIT_MAX_BUFFER });
  if (result.error || result.status !== 0) {
    throw new Error(`git credential approve failed: ${sanitize(result.error?.message || result.stderr || result.stdout || result.status)}`);
  }
}

function rejectCredential(repo) {
  const parsed = new URL(repo);
  if (parsed.protocol === 'file:') {
    return;
  }
  ...
  spawnSync('git', ['credential', 'reject'], { input, encoding: 'utf8', maxBuffer: GIT_MAX_BUFFER });
}
