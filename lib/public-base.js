...
function publishPublicBase(options = {}) {
  ...
  if (sidecar.status === 'mismatch') {
    const error = new Error('bundle sha256 sidecar does not match bundle content');
    error.exitCode = 5;
    throw error;
  }
  ...
  validateExistingPublishDir(publishDir, manifest.public_base_key, sha256File(bundle));
  ...
  writeText(shaTarget, `${sha256File(bundle)}  ${bundleName}\n`);
  ...
  const summary = {
    ...
    latest_path: norm(path.join(releaseDir, 'latest.json')),
    public_base_key: manifest.public_base_key,
    public_base_key_short: keyShort,
  ...
  core.writeJson(path.join(publishDir, 'publish-summary.json'), summary);
  return summary;
}
...
function runPublicBaseCli(args = [], options = {}) {
  ...
  try {
    ...
    if (command === 'publish') {
      const parsed = parseArgs(args.slice(1));
      const result = publishPublicBase({ ...parsed, repoRoot: options.repoRoot || parsed.repoRoot, cwd: options.cwd, env: options.env });
      stdout.write([
...
function validatePublishBranch(branch) {
  ...
  if (branch.split('/').some((part) => !part || part.startsWith('.') || part.endsWith('.') || part.endsWith('.lock'))) {
    throw new Error(`invalid public-base branch: ${branch}`);
  }
...
function publicBaseHelp() {
  return [
    ...
    '  ad-build public-base publish --repo <ad-build-public-base> --branch <branch> [--bundle <public-base.tar>]',
    '  ad-build public-base status',
