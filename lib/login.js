...
function login(options = {}) {
  ...
  const probe = probeGitLabSsh(key.privateKeyPath, options.env);
...
function probeGitLabSsh(privateKeyPath, env) {
  ...
  const ok = !result.error && (
    result.status === 0
    || /welcome|authenticated|successfully authenticated/i.test(message)
  );
  return {
    ok,
...
function helpText() {
  return [
    ...
    '',
    '默认行为:',
    '  配置 artifact overlay 仓库的 SSH 访问。',
    '  探测时强制使用当前选择的私钥，并启用 IdentitiesOnly，避免 ssh-agent 或其他 key 干扰。',
    ...
    '把打印出来的完整公钥添加到 GitLab SSH Keys 后，重新执行 ad-build login。',
    ''
  ].join('\n');
}
...
function renderLoginText(result) {
  const humanStatus = result.status === 'authenticated' ? '已通过' : '待添加密钥';
  const lines = [
    `overlay SSH 登录: ${humanStatus}`,
    `认证文件: ${result.auth_path}`,
    `产物仓库: ${result.artifact_repo_ssh}`,
    `需要添加的公钥文件: ${result.public_key_path}`,
    '',
    '请把下面这一整行添加到 GitLab -> SSH Keys:',
    result.public_key || '(missing)',
    ''
  ];
  if (result.status !== 'authenticated') {
    lines.push(`SSH 探测未通过: ${result.probe_message || `exit ${result.probe_exit_code}`}`);
    lines.push('确认添加的是上面这把公钥后，重新执行: ad-build login');
    lines.push('');
  } else {
    lines.push('SSH 探测已通过，可以继续执行 overlay pack/publish/use。');
    lines.push('');
  }
  return lines.join('\n');
}
