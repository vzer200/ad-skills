...
const OVERLAY_DIR = path.join(OUTPUT_DIR, 'overlay');
const OVERLAY_CACHE_DIR = path.join(OUTPUT_DIR, 'cache', 'artifact-overlay-repo');
const AUTH_PATH = path.join(OVERLAY_DIR, 'auth.json');
const CURRENT_PATH = path.join(OVERLAY_DIR, 'current.json');
const USE_SUMMARY_PATH = path.join(OVERLAY_DIR, 'use-summary.json');
...
function packOverlay(options = {}) {
  ...
  const outDir = path.resolve(repoRoot, options.out || path.join(OVERLAY_DIR, 'latest'));
...
function publishOverlay(options = {}) {
  ...
  const summary = {
    ...
    cache_path: normalizeOutputPath(path.relative(repoRoot, artifactRepo.path)),
    release: branch,
  ...
  core.writeJson(path.join(repoRoot, OVERLAY_DIR, 'publish-summary.json'), summary);
  return summary;
}
...
function useOverlay(options = {}) {
  ...
  const previous = readCurrent(repoRoot);
  const conflicts = findRestoreConflicts({ repoRoot, inventory, staging, manifest, previous, force: options.force });
  if (conflicts.length > 0) {
    core.writeJson(path.join(repoRoot, OVERLAY_DIR, 'use-conflicts.json'), {
      schema_version: 1,
      generated_at: core.nowIso(),
      conflicts
    });
    throw exitError(`overlay use would overwrite ${conflicts.length} local paths; inspect .ad-build/overlay/use-conflicts.json`, 5);
  }
  ...
  core.writeJson(path.join(repoRoot, CURRENT_PATH), current);
  const doctor = buildDoctorResult({ repoRoot, env: options.env, current });
  const summary = {
    schema_version: 1,
    status: doctor.overall_status === 'failed' ? 'not_ready' : 'ready',
    generated_at: core.nowIso(),
    release: branch,
    current_path: normalizeOutputPath(CURRENT_PATH),
    summary_path: normalizeOutputPath(USE_SUMMARY_PATH),
    artifact_sha256: actualSha,
  ...
  core.writeJson(path.join(repoRoot, USE_SUMMARY_PATH), summary);
  core.writeJson(path.join(repoRoot, OVERLAY_DIR, 'doctor.json'), doctor);
  return summary;
}
...
function runStatus(options = {}) {
  ...
  const summary = readOptionalJson(path.join(repoRoot, USE_SUMMARY_PATH));
  return {
    schema_version: 1,
    generated_at: core.nowIso(),
    ...
    current_path: fs.existsSync(path.join(repoRoot, CURRENT_PATH)) ? normalizeOutputPath(CURRENT_PATH) : null,
    use_summary_path: fs.existsSync(path.join(repoRoot, USE_SUMMARY_PATH)) ? normalizeOutputPath(USE_SUMMARY_PATH) : null,
...
function runDoctor(options = {}) {
  const repoRoot = resolveRepoRoot(options);
  const result = buildDoctorResult({ repoRoot, env: options.env });
  core.writeJson(path.join(repoRoot, OVERLAY_DIR, 'doctor.json'), result);
  return {
    ...result,
    output_path: normalizeOutputPath(path.join(OVERLAY_DIR, 'doctor.json'))
  };
}

function repairPaths(options = {}) {
  const repoRoot = resolveRepoRoot(options);
  const current = requireCurrent(repoRoot);
  ...
  core.writeJson(path.join(repoRoot, OVERLAY_DIR, 'repair-paths.json'), result);
  return result;
}

function repairDpdk(options = {}) {
  const repoRoot = resolveRepoRoot(options);
  const runDir = makeRunDir(repoRoot, 'repair-dpdk');
  ...
  const summary = {
    ...
    log_path: normalizeOutputPath(path.relative(repoRoot, logPath)),
    first_real_error: extractFirstRealError([{ path: logPath, label: normalizeOutputPath(path.relative(repoRoot, logPath)) }])
  };
...
function buildModule(options = {}) {
  ...
  const runDir = makeRunDir(repoRoot, `build-${moduleName}`);
  const env = {
    ...process.env,
    ...(options.env || {}),
    PREFIX_SOURCE: repoRoot
  ...
  const summary = {
    ...
    error_log_path: firstRealError?.source_file || normalizeOutputPath(path.relative(repoRoot, logPath)),
  ...
  core.writeJson(path.join(repoRoot, OVERLAY_DIR, 'last-build-summary.json'), summary);
  return summary;
}
...
function buildDoctorResult(options = {}) {
  ...
  const auth = readAuth(repoRoot);
  const current = options.current || readCurrent(repoRoot);
  const useSummary = readOptionalJson(path.join(repoRoot, USE_SUMMARY_PATH));
  const checks = [];
  const warnings = [];
...
function selectPackedOverlay(repoRoot, options) {
  const artifact = path.resolve(repoRoot, options.overlay || path.join(OVERLAY_DIR, 'latest', 'ad-artifact-overlay.tar.gz'));
...
function ensureArtifactRepo(repoRoot, options = {}, mode = {}) {
  ...
  const cachePath = path.join(repoRoot, OVERLAY_CACHE_DIR);
...
function readAuth(repoRoot) {
  return readOptionalJson(path.join(repoRoot, AUTH_PATH));
}

function readCurrent(repoRoot) {
  return readOptionalJson(path.join(repoRoot, CURRENT_PATH));
}

function requireCurrent(repoRoot) {
  const current = readCurrent(repoRoot);
  if (!current) {
...
function ensureManagedCachePathSafe(repoRoot, cachePath) {
  assertInside(repoRoot, cachePath, 'overlay cache path');
  let current = repoRoot;
  for (const part of normalizeOutputPath(path.relative(repoRoot, cachePath)).split('/')) {
    current = path.join(current, part);
    if (!fs.existsSync(current)) {
      continue;
    }
    const stat = fs.lstatSync(current);
    if (stat.isSymbolicLink()) {
      throw exitError(`overlay cache path contains symlink: ${normalizeOutputPath(path.relative(repoRoot, current))}`, 5);
    }
    if (!stat.isDirectory()) {
      throw exitError(`overlay cache path component is not a directory: ${normalizeOutputPath(path.relative(repoRoot, current))}`, 5);
    }
  }
}
...
function makeRunDir(repoRoot, prefix) {
  const dir = path.join(repoRoot, OVERLAY_DIR, 'runs', `${prefix}-${makeRunId()}`);
