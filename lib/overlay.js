...
const FORCE_BASELINE_CLEANUP_PATHS = [
  'obj',
  'app_bin',
  'apps/ad_appd_new/libs/dpdk/dpdk-stable-20.11.5/build',
...
const FORCE_BASELINE_PROTECTED_PATHS = [
  'source files outside declared build outputs',
  '.git/',
  'mkpacket/',
  'ssipacket/',
  'ad_packet/',
  ...
  try {
    ...
    if (parsed.command === 'use') {
      ...
      writeCliResult(stdout, parsed, result, [
        ...
        `阶段耗时: ${formatStageTimings(result.stage_timings)}`,
...
function measureStage(stageTimings, name, fn) {
  const startedAt = Date.now();
  try {
    return fn();
  } finally {
    stageTimings[`${name}_ms`] = Date.now() - startedAt;
  }
}

function formatStageTimings(stageTimings = {}) {
  const labels = [
    ['metadata_ms', 'metadata'],
    ['artifact_fetch_ms', 'artifact-fetch'],
    ['sha256_ms', 'sha256'],
    ['tar_safety_ms', 'tar-safety'],
    ['extract_ms', 'extract'],
    ['force_cleanup_ms', 'force-cleanup'],
    ['conflict_check_ms', 'conflict-check'],
    ['inventory_restore_ms', 'inventory-restore'],
    ['external_dependency_links_ms', 'external-links'],
    ['relocate_ms', 'relocate'],
    ['dpdk_repair_ms', 'dpdk-repair'],
    ['doctor_ms', 'doctor']
  ];
  return labels
    .filter(([key]) => Object.prototype.hasOwnProperty.call(stageTimings, key))
    .map(([key, label]) => `${label}=${formatDuration(stageTimings[key])}`)
    .join(', ');
}
...
function useOverlay(options = {}) {
  ...
  const metadata = measureStage(stageTimings, 'metadata', () => {
    const value = readRestoreMetadata(repoRoot, branch, options);
    assertManifestReleaseMatches('restore', branch, value.manifest, options);
    const metadataSourceIssue = sourceVerificationIssue(value.manifest, currentGit);
  ...
  const artifactRepo = measureStage(stageTimings, 'artifact_fetch', () => (
    metadata.artifactRepo.local
  ...
  const actualSha = measureStage(stageTimings, 'sha256', () => sha256File(artifactPath, { onProgress: hashProgress(options, '校验 overlay 包 sha256') }));
  ...
  try {
    ...
    measureStage(stageTimings, 'tar_safety', () => validateOverlayArchive(artifactPath, inventory));
    emitProgress(options, '正在解压 overlay 包');
    measureStage(stageTimings, 'extract', () => runTar(['-xzf', artifactPath, '-C', staging], options, '解压 overlay'));
    const previous = readCurrent(repoRoot, options);
    const forceCleanup = measureStage(stageTimings, 'force_cleanup', () => cleanupForceBaseline({
      repoRoot,
      inventory,
      manifest,
    ...
    const conflicts = measureStage(stageTimings, 'conflict_check', () => (
      findRestoreConflicts({ repoRoot, inventory, staging, manifest, previous, force: options.force })
    ));
    ...
    const restored = measureStage(stageTimings, 'inventory_restore', () => (
      restoreInventoryEntries({ repoRoot, inventory, staging, manifest, previous, force: options.force })
    ));
    const externalDependencyLinks = measureStage(stageTimings, 'external_dependency_links', () => restoreExternalDependencyLinks({
      repoRoot,
      dependencies: manifest.external_dependencies || [],
      force: options.force
    }));
    emitProgress(options, '正在修正旧工作区路径引用');
    const relocation = measureStage(stageTimings, 'relocate', () => relocateInventoryEntries({
      repoRoot,
      entries: inventory.entries || [],
      oldRoot: manifest.source_root_at_pack_time || DEFAULT_SOURCE_ROOT,
      newRoot: repoRoot
    }));
    const dpdkRepair = measureStage(stageTimings, 'dpdk_repair', () => maybeRepairDpdkAfterRestore({ ...options, repoRoot }));
    ...
    const current = {
      ...
      restored,
      external_dependency_links: externalDependencyLinks,
    ...
    const doctor = measureStage(stageTimings, 'doctor', () => (
      buildDoctorResult({ repoRoot, env: options.env, current, stateOptions: options, skipUseSummaryCheck: true })
    ));
    const summary = {
      schema_version: 1,
      ...
      external_dependency_links_restored: externalDependencyLinks.restored_count,
      external_dependency_links_skipped: externalDependencyLinks.skipped_count,
    ...
    summary.stage_timings.total_ms = summary.duration_ms;
    ...
    emitProgress(options, `阶段耗时: ${formatStageTimings(stageTimings)}`);
    return summary;
  } finally {
    cleanupTempDir(staging);
  }
...
function maybeRepairDpdkAfterRestore(options = {}) {
  ...
  try {
    ensureDpdkRepairPathSafe(repoRoot);
  } catch (error) {
    return {
      status: 'failed',
      error: error.message,
      exit_code: error.exitCode || 5
    };
  }
...
function repairDpdk(options = {}) {
  ...
  for (const rel of [
    ...
    ensureDestinationSafe(repoRoot, rel);
...
function ensureDpdkRepairPathSafe(repoRoot) {
  ensureDestinationSafe(repoRoot, 'apps/ad_appd_new/libs/dpdk/.ad-build-safe-probe');
}
...
function buildModule(options = {}) {
  ...
  const detectedLogError = extractFirstRealError([
  ...
  const firstRealError = exitCode === 0 ? null : detectedLogError;
  const nonfatalLogErrorMatch = exitCode === 0 && detectedLogError
    ? detectedLogError
    : null;
  const summary = {
    schema_version: 1,
    ...
    nonfatal_log_error_match: nonfatalLogErrorMatch,
    warnings: nonfatalLogErrorMatch
      ? [{
        name: 'nonfatal_log_error_match',
        message: `命令已成功，日志中的错误匹配作为非致命信息隐藏: ${nonfatalLogErrorMatch.message}`,
        source_file: nonfatalLogErrorMatch.source_file,
        line: nonfatalLogErrorMatch.line
      }]
      : [],
...
function buildDoctorResult(options = {}) {
  ...
  if (current) {
    ...
    for (const dependency of current.manifest.external_dependencies || []) {
      ...
      const linkCheck = checkExternalDependencyLink(repoRoot, dependency);
      if (linkCheck) {
...
function checkExternalDependencyLink(repoRoot, dependency) {
  if (!shouldRestoreExternalDependencyLink(dependency)) {
    return null;
  }
  const name = dependency.name || dependency.path || 'unknown';
  const target = path.join(repoRoot, dependency.path);
  const linkTarget = externalDependencyLinkTarget(dependency) || 'unknown';
  if (!pathExistsOrSymlink(target)) {
    return check(
      `external_dependency_link:${name}`,
      'failed',
      `外部系统依赖入口缺失: ${dependency.path} -> ${linkTarget}`
    );
  }
  if (externalDependencyLinkMatches(target, dependency)) {
    return check(
      `external_dependency_link:${name}`,
      'passed',
      `外部系统依赖入口已存在: ${dependency.path} -> ${linkTarget}`
    );
  }
  return check(
    `external_dependency_link:${name}`,
    'failed',
    `外部系统依赖入口不匹配: ${dependency.path} -> ${linkTarget}`
  );
}
  ...
  if (policy?.action === 'dependency') {
    return {
      ...
      item: {
        ...
        restore_link: Boolean(policy.restoreLink),
        required: true,
...
function isTopLevelBuildEntry(rel) {
  return SCAN_FILES.includes(rel) || /^(Makefile.*|app.*\.mk)$/.test(rel);
}
...
function restoreExternalDependencyLinks({ repoRoot, dependencies = [], force = false }) {
  let restoredCount = 0;
  let skippedCount = 0;
  let missingCount = 0;
  const conflicts = [];
  ...
  for (const dependency of dependencies) {
    ...
    ensureDestinationSafe(repoRoot, dependency.path);
    const target = path.join(repoRoot, dependency.path);
    const linkTarget = externalDependencyLinkTarget(dependency);
    const checkPath = externalDependencyCheckPath(dependency);
    if (!linkTarget || !checkPath || !fs.existsSync(checkPath)) {
      missingCount += 1;
      skippedCount += 1;
      continue;
    }
    if (pathExistsOrSymlink(target)) {
      ...
      if (stat.isDirectory()) {
        if (!force) {
          conflicts.push({ path: dependency.path, reason: 'target is a directory' });
          continue;
        }
        fs.rmSync(target, { recursive: true, force: true });
      } else {
        if (!force) {
          conflicts.push({ path: dependency.path, reason: 'existing path is not the declared external dependency link' });
          continue;
        }
        fs.rmSync(target, { force: true });
      }
    ...
    fs.mkdirSync(path.dirname(target), { recursive: true });
    fs.symlinkSync(linkTarget, target, externalDependencySymlinkType(checkPath));
    restoredCount += 1;
  }

  if (conflicts.length > 0) {
    const details = conflicts.map((item) => `${item.path}: ${item.reason}`).join('; ');
    throw exitError(`外部依赖入口软链存在冲突，确认覆盖时追加 --force: ${details}`, 5);
  }
...
function cleanupForceBaseline({ repoRoot, inventory, stateOptions = {}, enabled = false }) {
  const planPath = overlayStatePath(stateOptions, 'force-plan.json');
  const summaryPath = overlayStatePath(stateOptions, 'force-summary.json');
  if (!enabled) {
    return {
      enabled: false,
      deleted_count: 0,
  ...
  emitProgress(stateOptions, '--force 已启用，将强制恢复到 overlay 基线环境');
  emitProgress(stateOptions, '仅清理可由 overlay 恢复的编译产物、缓存和软链接，不会删除源码或未知目录');

  const cleanupPaths = FORCE_BASELINE_CLEANUP_PATHS.map((rel) => {
    const reason = forceCleanupReason(rel, inventory);
    return {
      path: rel,
      reason,
      action: reason ? 'remove_before_restore' : 'skip_not_declared'
    };
  });
  const plan = {
    schema_version: 1,
    generated_at: core.nowIso(),
    mode: 'restore_force_overlay_baseline',
    message: '--force 已启用，将强制恢复到 overlay 基线环境',
    note: '仅清理可由 overlay 恢复的编译产物、缓存和软链接，不会删除源码或未知目录。',
    cleanup_paths: cleanupPaths,
    protected_paths: FORCE_BASELINE_PROTECTED_PATHS
  };
  core.writeJson(planPath, plan);
  ...
  for (const item of cleanupPaths) {
    ...
    try {
      ...
      if (shouldExclude(rel)) {
        skipped.push({ path: rel, reason: 'excluded_root' });
        continue;
      }
      if (!item.reason) {
        skipped.push({ path: rel, reason: 'not_declared_by_manifest' });
        continue;
      }
      ensureDestinationSafe(repoRoot, rel);
      const target = path.join(repoRoot, rel);
      assertInside(repoRoot, target, 'restore force cleanup target');
      if (!pathExistsOrSymlink(target)) {
      ...
      fs.rmSync(target, { recursive: true, force: true });
      deleted.push({ path: rel, type, reason: item.reason });
    } catch (error) {
      skipped.push({ path: rel, reason: 'error', message: error.message });
    }
  }
  ...
  const summary = {
    ...
    status: 'completed',
    mode: 'restore_force_overlay_baseline',
    plan_path: normalizeOutputPath(planPath),
    summary_path: normalizeOutputPath(summaryPath),
...
function forceCleanupReason(rel, inventory = {}) {
  const normalized = normalizeOutputPath(rel).replace(/\/+$/, '');
  if (inventoryHasEntryUnderPath(inventory, normalized)) {
    return 'overlay_inventory';
  }
  if (normalized === 'apps/ad_appd_new/libs/dpdk/tmp_install') {
    return 'dpdk_repair_cache';
  }
  return null;
}

function inventoryHasEntryUnderPath(inventory = {}, rel) {
  const prefix = `${normalizeOutputPath(rel).replace(/\/+$/, '')}/`;
  return (inventory.entries || []).some((entry) => {
    const entryPath = normalizeOutputPath(entry.path || '');
    return entryPath === rel || entryPath.startsWith(prefix);
  });
}

function shouldRestoreExternalDependencyLink(dependency) {
  return Boolean(dependency?.restore_link)
    || (dependency?.path === 'include/lua' && normalizeOutputPath(dependency?.link_target || '').startsWith('/usr/local/include/luajit-2.1'));
}

function externalDependencyLinkTarget(dependency) {
  return dependency?.link_target || dependency?.check_path || dependency?.resolved_path || null;
}

function externalDependencyCheckPath(dependency) {
  return dependency?.check_path || dependency?.resolved_path || dependency?.link_target || null;
}

function externalDependencySymlinkType(checkPath) {
  try {
    return fs.existsSync(checkPath) && fs.lstatSync(checkPath).isDirectory() ? 'dir' : 'file';
  } catch {
    return 'file';
  }
}

function externalDependencyLinkMatches(target, dependency) {
  try {
    const stat = fs.lstatSync(target);
    const linkTarget = normalizeOutputPath(externalDependencyLinkTarget(dependency) || '').replace(/\/+$/, '');
    if (stat.isSymbolicLink()) {
      const current = normalizeOutputPath(fs.readlinkSync(target)).replace(/\/+$/, '');
      return current === linkTarget;
    }
    if (stat.isDirectory() && dependency.path === 'include/lua') {
      return fs.existsSync(path.join(target, 'lua.h'));
    }
  } catch {
    return false;
  }
...
function classifyEntry(rel, gitStatus) {
  ...
  if (isBuildMetadataPath(rel) || isBuildOutputPath(rel) || isTopLevelBuildEntry(rel)) {
