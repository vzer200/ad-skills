...
function packOverlay(options = {}) {
  ...
  try {
    ...
    const entries = collectPackEntries(repoRoot, { sourceRoot, progress: options.progress });
...
function ensureArtifactRepo(repoRoot, options = {}, mode = {}) {
  ...
  if (branch) {
    if (mode.forPublish) {
      ...
      const fetch = gitProgress(cachePath, ['fetch', '--progress', '--depth=1', '--filter=blob:none', 'origin', branch], authEnv, options);
      if (fetch.error || fetch.status !== 0) {
        throw exitError(`获取产物 manifest 元数据失败: ${gitFailure(fetch)}`, 4);
      }
    } else {
...
function resolveInternalSymlinkTarget({ repoRoot, sourceRoot, linkPath, linkTarget }) {
  ...
  const normalizedSourceRoot = normalizeOutputPath(sourceRoot || '');
