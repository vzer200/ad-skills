...
function packOverlay(options = {}) {
  ...
  try {
    ...
    const summary = {
      ...
      pack_rules_version: PACK_RULES_VERSION,
      warnings: entries.length === 0 ? [{ message: '没有找到 overlay 产物文件' }] : []
    };
...
function useOverlay(options = {}) {
  ...
  const artifactRepo = ensureArtifactRepo(repoRoot, options, { forUse: true, branch });
...
function sourceVerificationIssue(manifest, currentGit) {
  ...
  if (!currentGit?.branch) {
    missingCurrent.push('source_branch');
  }
  if (!currentGit?.commit) {
    missingCurrent.push('source_commit');
  }
  if (missingCurrent.length > 0) {
    return {
      message: `当前 AD 工作区无法读取源码 Git 信息: ${missingCurrent.join(', ')}`,
      forceable: false
    };
  }
...
function sourceDrift(manifest, currentGit) {
  const compareUrl = sourceCompareUrl(manifest, currentGit);
  const compareHint = compareUrl ? `；GitLab compare: ${compareUrl}` : '';
  if (manifest.source_branch && currentGit.branch && manifest.source_branch !== currentGit.branch) {
    return `源码分支不一致: overlay 来自 ${manifest.source_branch}，当前工作区是 ${currentGit.branch}${compareHint}`;
  }
  if (manifest.source_commit && currentGit.commit && manifest.source_commit !== currentGit.commit) {
    return `源码 commit 不一致: overlay 来自 ${manifest.source_commit}，当前工作区是 ${currentGit.commit}${compareHint}`;
  }
  return null;
}

function sourceCompareUrl(manifest = {}, currentGit = {}) {
  const webUrl = gitRemoteWebUrl(manifest.source_repo_url || currentGit.remote_url);
  const from = manifest.source_commit || manifest.source_branch;
  const to = currentGit.commit || currentGit.branch;
  if (!webUrl || !from || !to) {
    return null;
  }
  return `${webUrl}/-/compare/${encodeCompareRef(from)}...${encodeCompareRef(to)}`;
}

function gitRemoteWebUrl(remoteUrl) {
  const value = String(remoteUrl || '').trim();
  if (!value) {
    return null;
  }

  const scpLike = /^git@([^:]+):(.+?)(?:\.git)?$/.exec(value);
  if (scpLike) {
    return `https://${scpLike[1]}/${scpLike[2]}`;
  }

  const sshLike = /^ssh:\/\/git@([^/]+)\/(.+?)(?:\.git)?$/.exec(value);
  if (sshLike) {
    return `https://${sshLike[1]}/${sshLike[2]}`;
  }

  const httpLike = /^(https?:\/\/.+?)(?:\.git)?\/?$/.exec(value);
  return httpLike ? httpLike[1] : null;
}
