...
function tracked(repoRoot, baseRef = 'HEAD') { const r = git(repoRoot, ['diff', '--name-status', '-z', '--find-renames=50%', '--find-copies=50%', baseRef, '--']); if (r.error || r.status !== 0) throw new Error(`git diff failed: ${gitFailure(r)}`); return parseNameStatus(r.stdout); }
function untracked(repoRoot) { const r = git(repoRoot, ['ls-files', '--others', '--exclude-standard', '-z']); if (r.error || r.status !== 0) throw new Error(`git ls-files failed: ${gitFailure(r)}`); return nul(r.stdout).map(norm); }
...
function git(repoRoot, args) { return spawnSync('git', args, { cwd: repoRoot, encoding: 'utf8', maxBuffer: GIT_MAX_BUFFER }); }
...
function gitFailure(result) { return result.error?.message || trim(result.stderr) || (result.signal ? `signal ${result.signal}` : `exit ${result.status}`); }
