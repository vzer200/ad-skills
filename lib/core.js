...
function normalizeForJson(value) {
  ...
  if (value && typeof value === 'object') {
    ...
    if (proto !== Object.prototype && proto !== null) {
      throw new TypeError(`JSON 规范化不支持该对象类型: ${value.constructor?.name || 'Object'}`);
    }
...
function safeDigestKey(digest) {
  if (typeof digest !== 'string') {
    throw new TypeError('digest 无效：需要 sha256 字符串');
  }

  const match = /^(?:sha256:)?([a-f0-9]{64})$/.exec(digest);
  if (!match) {
    throw new Error('digest 无效：需要 sha256:[a-f0-9]{64} 或 [a-f0-9]{64}');
  }
...
function computeRefKey(symbolicRef, commit) {
  if (!symbolicRef && !commit) {
    throw new Error('computeRefKey 需要 symbolicRef 或 commit');
  }
