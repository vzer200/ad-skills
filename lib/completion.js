...
const OPTIONS = {
  'public-base': ['--json', '--branch', '--bundle', '--out', '--config', '--allow-partial', '--allow-unproven', '--integrity-only', '--push', '--token-stdin', '--remove-cache'],
...
_ad_build() {
  local -a commands public_base bundle completion inventory image skill auth_commands public_base_opts bundle_opts completion_opts inventory_opts image_opts login_opts logout_opts skill_opts default_opts
  commands=(${top})
...
function describeTopCommand(command) {
  const descriptions = {
    login: 'log in to public-base',
    logout: 'log out from public-base',
